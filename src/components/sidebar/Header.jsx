...
const Header = ({ activeTab, setActiveTab, open, setOpen }) => {
    ...
    return (
        <div className={`px-md-120 position-absolute position-sm-relative z-200 transition px-sm-96 min-vh-100 ${open ? "start-0" : "left100"}`}>
            <div className="top-0 max-w-sm-192 max-w-md-240 max-w-240 w-100 d-flex flex-column justify-content-between min-vh-100 position-sm-fixed bg-black p-sm-3 p-4 p-md-4"
                style={{ left: isLargeScreen ? 'calc((100vw - 1920px) / 2)' : '0%' }}>
                <div className="d-flex flex-column">
                    <a href="/" className="w-fit mx-auto w-sm-75 mt-4"><img src={logo} alt="logo" className='w-100' /></a>
                    <div className="d-flex flex-column mt-64 gap-32">
                        {TABS_DATA.map((tab, idx) => (
                            <div key={idx} className="d-flex align-items-center gap-2 gap-md-20">
                                <span className={`w-6 h-6 rounded-full ${activeTab === idx ? "bg-off-yellow" : "bg-black"}`} />
                                <a href={`/${tab.title.toLowerCase()}`}
                                    className={`text-base d-flex align-items-center gap-2 gap-md-20 fw-normal ${activeTab === idx ? "text-off-yellow" : "text-white"}`}
